...
class Trainer:
    ...
    def adapt_weight(self, iter_now, iter_max_default, iter_max_input, weight_loss, weight_value=10.0, high_value=1.0, low_value=0.0):
        ...
        adaptive_weight = (2. / (1. + np.exp(-weight * p)) - 1) * (high-low) + low   # adaptive_weight會從0變到1
    ...
    def train(self, model, model_dir, results_dir, batch_gen_source, batch_gen_target, device, args):
        ...
        for epoch in range(resume_epoch, num_epochs):
            ...
            while batch_gen_source.has_next():
                ...
                # ====== Feed-forward data ====== #
                # prepare inputs
                # input: (batch, dim, frame#)
                # label: (batch, frame#)
                # f_label: (batch, frame#)
                # mask: (batch, class#, frame#)
                input_source, label_source, f_label_source, mask_source = batch_gen_source.next_batch(batch_size_source, 'source')
                input_source, label_source, f_label_source, mask_source = input_source.to(device), label_source.to(device), f_label_source.to(device), mask_source.to(device)

                # drop some source frames (including labels) for semi-supervised learning
                input_source, label_source, f_label_source, mask_source = self.ctrl_video_length(input_source, label_source, f_label_source, mask_source, ratio_source)

                # drop source labels only
                label_source_new, f_label_source_new, mask_source_new = self.ctrl_video_label_length(label_source, f_label_source, mask_source, ratio_label_source)

                input_target, label_target, _, mask_target = batch_gen_target.next_batch(batch_size_target, 'target')
                ...
                # a_pred: (batch, stage#, class#, frame#)
                # f_pred: (batch, stage#, frame#)
                # feat: (batch, stage#, dim, frame#)
                # pred_d: (batch x frame#, stage#, class#, 2)
                # pred_d_video: (batch x seg#, stage#, 2)
                pred_source, a_pred_source, f_pred_source, f_pred_target, prob_source, feat_source, pred_target, prob_target, feat_target, \
                ...
                for s in range(num_stages):
                    ...
                    # classify the predictions from foreground attention features
                    a_p = a_pred_source[:, s, :, :]  # select one stage --> (batch, class#, frame#) 
                    if fore_attn == 'Y':
                        loss += self.ce(a_p.transpose(2, 1).contiguous().view(-1, self.num_classes), label_source_new.view(-1)) 
                        loss += alpha * torch.mean(torch.clamp(self.mse(F.log_softmax(a_p[:, :, 1:], dim=1), F.log_softmax(a_p.detach()[:, :, :-1], dim=1)), min=0, max=tau ** 2) * mask_source_new[:, :, 1:])
                    
                    # foreground-background clssification
                    f_p = f_pred_source[:, s, :]  # select one stage --> (batch, frame#)
                    if place_adv[s] == 'Y' and fore_attn == 'Y':
                        loss += 0.2 * self.bce(f_p.contiguous().view(-1), f_label_source_new.view(-1))
                ...
                if use_target != 'none':
                    ...
                    if DA_ens != 'none':  # get multiple target outputs
                        _, _, _, _, _, _, _, prob_target, _, _, _, _, _, _, _, _, prob_target_2 \
                    ...
                    for s in range(num_stages):
                        ...
                        if DA_adv == 'rev_grad':
                            if place_adv[s] == 'Y':
                                ...
                                for c in range(num_class_domain):
                                    ...
                                    # # use foreground socre as a weight applied on the adverial loss
                                    # f_pred_source_stage = f_pred_source[:, s, :].view(-1)  # (batch x frame#)
                                    # f_pred_target_stage = f_pred_target[:, s, :].view(-1)  
                                    # f_pred_stage = torch.cat((f_pred_source_stage, f_pred_target_stage), dim=0)
                                    # loss_adv_class *= f_pred_stage.detach()
                        ...
                        if 'rev_grad' in DA_adv_video:
                            if place_adv[s] == 'Y':
                                f_pred_target_stage = f_pred_target[:, s, :].mean(dim=1)  # (batch)
                                f_pred_source_stage = f_pred_source[:, s, :].mean(dim=1)
                                f_pred_stage = torch.cat((f_pred_target_stage, f_pred_source_stage), dim=0)  

                                loss_adv_video = self.ce_d(pred_d_video_stage, label_d_video_stage)
                                loss_adv_video *= f_pred_stage
            ...
            if verbose:
                print("[epoch %d]: epoch loss = %f,   acc (S) = %f,   acc (T) = %f,   lr = %f,   beta = %f,   nu = %f" % (epoch + 1, epoch_loss / num_iter_epoch, acc_epoch_source, acc_epoch_target, learning_rate, beta_in[0] , nu_in))  # uncomment for debugging
    ...
    def ctrl_video_length(self, input_data, label, f_label, mask, ratio_length):
        # shapes:
        ...
        f_label_filtered = f_label[:, id_keep]
        mask_filtered = mask[:, :, id_keep]

        return input_data_filtered, label_filtered, f_label_filtered, mask_filtered

    def ctrl_video_label_length(self, label, f_label, mask, ratio_length):
        # shapes:
        # label: (batch, frame#)
        # f_label: (batch, frame#)
        # mask: (batch, class#, frame#)
        ...
        f_label_new[:, id_drop] = 0

        return label_new, f_label_new, mask_new
